...
public class MaskedEditText extends AppCompatEditText implements TextWatcher {
    ...
    public MaskedEditText(@NonNull Context context, @NonNull AttributeSet attrs) {
        ...
        boolean enableImeAction = attributes
                .getBoolean(R.styleable.MaskedEditText_enable_ime_action, false);

        String representation = attributes
                .getString(R.styleable.MaskedEditText_char_representation);

        if (representation == null) {
            charRepresentation = CONSTANTS.NUM_SIGN;
        } else {
    ...
    private void cleanUp() {
        ...
        editingAfter = true;
        if (hasHint() && rawText.getLength() == 0) {
    ...
    private void generatePositionArrays() {
        ...
        if (charsInMaskAux.indexOf(CONSTANTS.SPACE) < 0) {
    ...
    private CharSequence makeMaskedTextWithHint() {
        ...
        for (int i = 0; i < mask.length(); i++) {
            ...
            if (mtrv == -1) {
                ...
                if (mtrv < rawText.getLength()) {
                    ssb.append(rawText.charAt(mtrv));
                } else {
                    ssb.append(getHint().charAt(maskToRaw[i]));
                }
            }
            if ((keepHint && rawText.getLength() < rawToMask.length && i >= rawToMask[rawText
                    .getLength()]) || (!keepHint && i >= maskFirstChunkEnd)) {
    ...
    private String makeMaskedText() {
        ...
        if (rawText.getLength() < rawToMask.length) {
            maskedTextLength = rawToMask[rawText.getLength()];
        } else {
    ...
    private int lastValidPosition() {
        if (rawText.getLength() == maxRawLength) {
            return rawToMask[rawText.getLength() - 1] + 1;
        }
        return nextValidPosition(rawToMask[rawText.getLength()]);
    }
    ...
    public MaskedEditText(@NonNull Context context, @NonNull AttributeSet attrs, int defStyle) {
    ...
    //todo change variable names very questionable
    ...
    @Override
    public void afterTextChanged(@NonNull Editable editableString) {
        if (!editingAfter && editingBefore && editingOnChanged) {
            editingAfter = true;
            if (hasHint() && (keepHint || rawText.getLength() == 0)) {
